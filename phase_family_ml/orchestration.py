...
def experiment_dirs(sequence_root: Path) -> list[Path]:
    """Return experiment directories that contain a counter-sequence manifest."""

    if not sequence_root.exists():
        return []
    return sorted(path for path in sequence_root.iterdir() if path.is_dir() and (path / "counter_sequence_manifest.json").exists())
